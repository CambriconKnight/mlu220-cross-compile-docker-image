# 1. CNCodec-MLU220Edge交叉编译与验证
MLU220交叉编译Docker镜像编译生成后，接下来可基于此镜像进行MLU220Edge实例程序的交叉编译与验证：

启动容器 >> 交叉编译 >> MLU220SOM验证

以下基于寒武纪运行时库[CNCodec](http://www.cambricon.com/docs/cncodec/user_guide_html)进行交叉编译与验证.
...
## 1.2. 交叉编译
以下是基于CNCodec进行交叉编译，详细交叉编译过程参考[CNCodec-MLU220Edge交叉编译脚本](./build-cncodec-mlu220edge.sh)。
```bash
#进入cncodec目录:在容器中映射目录下，进行编译，方便文件共享。
cd /home/cam/cncodec
#启动CNCodec-MLU220Edge一键化交叉编译脚本
#编译完成后，会在本目录下生成一个部署包，文件默认名称是cncodec_mlu220edge.tar.gz
./build-cncodec-mlu220edge.sh
#CNCodec-MLU220Edge生成后，可以直接拷贝部署到MLU220SOM上进行验证。(以实际IP为准替换)
scp cncodec_mlu220edge.tar.gz root@192.168.1.110:/cambricon/
#CNCodec-MLU220Edge生成后，也可以拷贝到NFS服务器挂载目录，采用挂载上位机NFS目录的方式，进⾏交互开发⼯作。
cp cncodec_mlu220edge.tar.gz /data/nfs/
```
...
cd /cambricon/nfs
ls -la cncodec_mlu220edge.tar.gz
#MLU220Edge板卡上解压cncodec_mlu220edge.tar.gz(上位机器上解压更快)
tar zxvf cncodec_mlu220edge.tar.gz
#进入应用实例子目录
cd ./cncodec_mlu220edge
